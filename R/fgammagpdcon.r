#' @export
#' 
#' @title MLE Fitting of Gamma Bulk and GPD Tail Extreme Value Mixture Model with Continuity Constraint
#'
...
#'
#' @param pvector vector of initial values mixture model parameters (\code{gshape}, \code{gscale}, \code{u}, \code{xi}) or \code{NULL}
#' @inheritParams fgammagpd
#' 
#' @details The extreme value mixture model with gamma bulk and GPD tail with a continuity constraint is 
...
#' gamma distribution \eqn{\phi_u = 1 - H(u)}. When \code{phiu=FALSE} then the tail
...
#' \code{allparams}: \tab vector of MLE of model parameters, including \code{sigmau} and \code{phiu}\cr
#' \code{allse}: \tab vector of standard error of all parameters, including \code{sigmau} and \code{phiu}\cr
...
#' \code{fgammagpdcon} to see the default formulae used. The mixture model fitting can be
...
#' 
#' @references
#' \url{http://en.wikipedia.org/wiki/Camma_distribution}
#' 
...
fgammagpdcon <- function(x, phiu = TRUE, pvector = NULL, std.err = TRUE, method = "BFGS",
  ...
  nllh = nlgammagpdcon(pvector, x = x, phiu = phiu, finitelik = finitelik)
  if (is.infinite(nllh))
    stop("initial parameter values are invalid")

  fit = optim(par = as.vector(pvector), fn = nlgammagpdcon, x = x, phiu = phiu, finitelik = finitelik,
  ...
  list(call = call, x = as.vector(x), init = as.vector(pvector), optim = fit,
    conv = conv, cov = invhess, mle = fit$par, se = se, rate = phiu, nllh = fit$value,
       ...
       gshape = gshape, gscale = gscale, u = u, sigmau = sigmau, xi = xi, phiu = phiu)
}
