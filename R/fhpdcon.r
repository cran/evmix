...
#' value mixture modelwith a single continuiuty constraint
#'
#' @param pvector    vector of initial values of mixture model parameters (\code{nmean}, \code{nsd}, \code{u}, \code{xi}) or \code{NULL}
...
# maximum likelihood fitting for hybrid Pareto with a single continuity constraint
fhpdcon <- function(x, pvector = NULL, std.err = TRUE, method = "BFGS",
  ...
  if (is.null(pvector)) {
    pvector[1] = mean(x)
    pvector[2] = sd(x)
    pvector[3] = quantile(x, 0.9)
    initfgpd = fgpd(x, u = as.vector(pvector[3]), std.err = std.err)
  ...
  nllh = nlhpdcon(pvector, x = x, finitelik = finitelik)
  if (is.infinite(nllh))
    stop("initial parameter values are invalid")

  fit = optim(par = as.vector(pvector), fn = nlhpdcon, x = x, finitelik = finitelik,
  ...
  list(call = call, x = as.vector(x), init = as.vector(pvector), optim = fit,
    ...
    allparam = c(fit$par, sigmau, phiu), allse = c(se, NA, NA), n = n,
