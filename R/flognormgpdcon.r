#' @export
#' 
#' @title MLE Fitting of Log-Normal Bulk and GPD Tail Extreme Value Mixture Model with Continuity Constraint
#'
...
#'
#' @param pvector vector of initial values of mixture model parameters (\code{lnmean}, \code{lnsd}, \code{u}, \code{xi}) or \code{NULL}
#' @inheritParams flognormgpd
#' 
#' @details The extreme value mixture model with log-normal bulk and GPD tail with
#' continuity constraint is fitted to the entire dataset using maximum likelihood estimation.
#' The estimated parameters, variance-covariance matrix and their standard errors are
...
#'
#' @seealso \code{\link[evmix:flognormgpd]{flognormgpd}}, 
#' \code{\link[evmix:llognormgpd]{llognormgpd}},
...
flognormgpdcon <- function(x, phiu = TRUE, pvector = NULL, std.err = TRUE, method = "BFGS",
  ...
  nllh = nllognormgpdcon(pvector, x = x, phiu = phiu, finitelik = finitelik)
  if (is.infinite(nllh))
    stop("initial parameter values are invalid")

  fit = optim(par = as.vector(pvector), fn = nllognormgpdcon, x = x, phiu = phiu, finitelik = finitelik,
