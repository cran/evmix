#' @export
#' 
#' @title MLE Fitting of Normal Bulk and GPD Tail Extreme Value Mixture Model with Continuity Constraint
#'
...
#'
#' @param pvector vector of initial values mixture model parameters (\code{nmean}, \code{nsd}, \code{u}, \code{xi}) or \code{NULL}
#' @inheritParams fnormgpd
#' 
#' @details The extreme value mixture model with normal bulk and GPD tail with a continuity constraint is 
...
#' \code{allparams}: \tab vector of MLE of model parameters, including \code{phiu} and \code{sigmau}\cr
#' \code{allse}: \tab vector of standard error of all parameters, including \code{phiu} and \code{sigmau}\cr
...
#' \code{fnormgpdcon} to see the default formulae used. The mixture model fitting can be
...
fnormgpdcon <- function(x, phiu = TRUE, pvector = NULL, std.err = TRUE, method = "BFGS",
  ...
  nllh = nlnormgpdcon(pvector, x = x, phiu = phiu, finitelik = finitelik)
  if (is.infinite(nllh))
    stop("initial parameter values are invalid")

  fit = optim(par = as.vector(pvector), fn = nlnormgpdcon, x = x, phiu = phiu, finitelik = finitelik,
