...
#' \code{fweibullgpd} to see the default formulae used. The mixture model fitting can be
...
fweibullgpd <- function(x, phiu = TRUE, pvector = NULL, std.err = TRUE, method = "BFGS",
  ...
  nllh = nlweibullgpd(pvector, x = x, phiu = phiu, finitelik = finitelik)
  if (is.infinite(nllh))
    stop("initial parameter values are invalid")

  fit = optim(par = as.vector(pvector), fn = nlweibullgpd, x = x, phiu = phiu, finitelik = finitelik,
