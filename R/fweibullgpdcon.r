#' @export
#' 
#' @title MLE Fitting of Weibull Bulk and GPD Tail Extreme Value Mixture Model with Continuity Constraint
#'
...
#'
#' @param pvector vector of initial values mixture model parameters (\code{wshape}, \code{wscale}, \code{u}, \code{sigmau}, \code{xi}) or \code{NULL}
...
#' Weibull distribution \eqn{\phi_u = 1 - H(u)}. When \code{phiu=FALSE} then the tail
...
#' \code{fweibullgpdcon} to see the default formulae used. The mixture model fitting can be
...
fweibullgpdcon <- function(x, phiu = TRUE, pvector = NULL, std.err = TRUE, method = "BFGS",
  ...
  nllh = nlweibullgpdcon(pvector, x = x, phiu = phiu, finitelik = finitelik)
  if (is.infinite(nllh))
    stop("initial parameter values are invalid")

  fit = optim(par = as.vector(pvector), fn = nlweibullgpdcon, x = x, phiu = phiu, finitelik = finitelik,
  ...
  list(call = call, x = as.vector(x), init = as.vector(pvector), optim = fit,
    conv = conv, cov = invhess, mle = fit$par, se = se, rate = phiu, nllh = fit$value,
       ...
       wshape = wshape, wscale = wscale, u = u, sigmau = sigmau, xi = xi, phiu = phiu)
}
