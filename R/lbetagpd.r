#' @export
#' 
#' @title Log-likelihood of beta Bulk and GPD Tail Extreme Value Mixture Model
#'
...
#' beta bulk and GPD tail, as used in the maximum likelihood fitting function 
#' \code{\link[evmix:fbetagpd]{fbetagpd}}.
#' 
#' Non-positive data are ignored. Values above 1 must come from GPD component, as
#' threshold \code{u<1}..
#' 
#' They are designed to be used for MLE in \code{\link[evmix:fbetagpd]{fbetagpd}}
#' but are available for wider usage, e.g. constructing your own extreme value
#' mixture models.
#' 
#' See \code{\link[evmix:fbetagpd]{fbetagpd}} and \code{\link[evmix:fgpd]{fgpd}}
#' for full details.
#' 
#' Log-likelihood calculations are carried out in
#' \code{\link[evmix:lbetagpd]{lbetagpd}}, which takes parameters as inputs in
#' the same form as distribution functions. The negative log-likelihood is a
#' wrapper for \code{\link[evmix:lbetagpd]{lbetagpd}}, designed towards making
...
#' 
#' Unlike the distribution functions \code{\link[evmix:betagpd]{betagpd}} the 
...
#' tail fraction is specified by beta distribution \eqn{\phi_u = 1 - H(u)}, or 
...
#' 
#' The function \code{\link[evmix:lbetagpd]{lbetagpd}} carries out the calculations
#' for the log-likelihood directly, which can be exponentiated to give actual
#' likelihood using (\code{log=FALSE}).
#' 
#' @return \code{\link[evmix:lbetagpd]{lbetagpd}} gives (log-)likelihood and 
#' \code{\link[evmix:lbetagpd]{nlbetagpd}} gives the negative log-likelihood.
#'  
...
#' \code{\link[evmix:lbetagpd]{lbetagpd}} and \code{\link[evmix:lbetagpd]{nlnormgpd}}. 
#' The \code{\link[evmix:lbetagpd]{lbetagpd}} also has the usual defaults for
#' the other parameters, but \code{\link[evmix:lbetagpd]{nlbetagpd}} has no defaults.
#' 
...
lbetagpd <- function(x, bshape1 = 1, bshape2 = 1, u = qbeta(0.9, bshape1, bshape2),
  ...
  lparams = c(length(bshape1), length(bshape2), length(u), length(sigmau), length(xi), length(phiu))
