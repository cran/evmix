#' @export
#' 
#' @title Log-likelihood of gamma Bulk and GPD Tail Extreme Value Mixture Model with Continuity Constraint
#'
...
#' \code{\link[evmix:fgammagpdcon]{fgammagpdcon}}.
#' 
#' They are designed to be used for MLE in \code{\link[evmix:fgammagpdcon]{fgammagpdcon}}
#' but are available for wider usage, e.g. constructing your own extreme value
#' mixture models.
#' 
#' Negative data are ignored.
#' 
#' See \code{\link[evmix:fgammagpdcon]{fgammagpdcon}} and \code{\link[evmix:fgpd]{fgpd}}
#' for full details.
#' 
#' Log-likelihood calculations are carried out in
#' \code{\link[evmix:lgammagpdcon]{lgammagpdcon}}, which takes parameters as inputs in
#' the same form as distribution functions. The negative log-likelihood is a
#' wrapper for \code{\link[evmix:lgammagpdcon]{lgammagpdcon}}, designed towards making
...
#' 
#' Unlike the distribution functions \code{\link[evmix:gammagpdcon]{gammagpdcon}} the 
...
#' 
#' The function \code{\link[evmix:lgammagpdcon]{lgammagpdcon}} carries out the calculations
#' for the log-likelihood directly, which can be exponentiated to give actual
#' likelihood using (\code{log=FALSE}).
#' 
#' @return \code{\link[evmix:lgammagpdcon]{lgammagpdcon}} gives (log-)likelihood and 
#' \code{\link[evmix:lgammagpdcon]{nlgammagpdcon}} gives the negative log-likelihood.
#'  
...
#' \code{\link[evmix:lgammagpdcon]{lgammagpdcon}} and \code{\link[evmix:lgammagpdcon]{nlnormgpd}}. 
#' The \code{\link[evmix:lgammagpdcon]{lgammagpdcon}} also has the usual defaults for
#' the other parameters, but \code{\link[evmix:lgammagpdcon]{nlgammagpdcon}} has no defaults.
#' 
...
#'
#' @seealso \code{\link[evmix:lgammagpd]{lgammagpd}}, \code{\link[evmix:lgpd]{lgpd}} and \code{\link[evmix:gpd]{gpd}}
...
lgammagpdcon <- function(x, gshape = 1, gscale = 1, u = qgamma(0.9, gshape, 1/gscale), 
  ...
  lparams = c(length(gshape), length(gscale), length(u), length(xi), length(phiu))
  ...
  if ((gscale <= 0) | (gshape <= 0) | (u <= 0) | (u <= min(x)) | (u >= max(x))) {
    ...
    du = dgamma(u, shape = gshape, scale = gscale)
    sigmau = phiu / (phib * du)
