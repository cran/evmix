#' @export
#' 
#' @title Log-likelihood of Hybrid Pareto Extreme Value Mixture Model with Single Continuity Constraint
#'
#' @description Log-likelihood and negative log-likelihood for the hybrid Pareto extreme
#' value mixture model with a single continuity constraint
#'
...
#' as used in the maximum likelihood fitting function \code{\link[evmix:fhpdcon]{fhpdcon}}.
#' 
#' They are designed to be used for MLE in \code{\link[evmix:fhpdcon]{fhpdcon}}
...
#' \code{\link[evmix:lhpdcon]{lhpdcon}}, which takes parameters as inputs in
#' the same form as distribution functions. The negative log-likelihood is a
#' wrapper for \code{\link[evmix:lhpdcon]{lhpdcon}}, designed towards making
#' it useable for optimisation (e.g. parameters are given a vector as first
#' input).
#' 
#' The function \code{\link[evmix:lhpdcon]{lhpdcon}} carries out the calculations
#' for the log-likelihood directly, which can be exponentiated to give actual
#' likelihood using (\code{log=FALSE}).
#' 
#' @return \code{\link[evmix:lhpdcon]{lhpdcon}} gives (log-)likelihood and 
#' \code{\link[evmix:lhpdcon]{nlhpdcon}} gives the negative log-likelihood.
#'  
...
#' 
#' The \code{\link[evmix:lhpdcon]{lhpdcon}} also has the usual defaults for
#' the other parameters, but \code{\link[evmix:lhpdcon]{nlhpdcon}} has no defaults.
#' 
...
#'
#' @seealso \code{\link[evmix:lhpd]{lhpd}}, \code{\link[evmix:lnormgpdcon]{lnormgpdcon}}, 
...
# log-likelihood function for hybrid Pareto with a single continuity constraint
# will not stop evaluation unless it has to
lhpdcon <- function(x, nmean = 0, nsd = 1, u = qnorm(0.9, nmean, nsd), xi = 0, log = TRUE) {
  ...
  lparams = c(length(nmean), length(nsd), length(u), length(xi))
  ...
  if ((nsd <= 0) | (u <= min(x)) | (u >= max(x))) {
    ...
    # Need to catch invalid normal parameters by checking normal density at u is not zero
    # could replace this with any(!is.finite(c(sigmau, phiu))) but this too general
