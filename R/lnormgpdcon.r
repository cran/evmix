#' @export
#' 
#' @title Log-likelihood of Normal Bulk and GPD Tail Extreme Value Mixture Model with Continuity Constraint
#'
...
#' \code{\link[evmix:fnormgpdcon]{fnormgpdcon}}.
#' 
#' They are designed to be used for MLE in \code{\link[evmix:fnormgpdcon]{fnormgpdcon}}
#' but are available for wider usage, e.g. constructing your own extreme value
#' mixture models.
#' 
#' See \code{\link[evmix:fnormgpdcon]{fnormgpdcon}} and \code{\link[evmix:fgpd]{fgpd}}
#' for full details.
#' 
#' Log-likelihood calculations are carried out in
#' \code{\link[evmix:lnormgpdcon]{lnormgpdcon}}, which takes parameters as inputs in
#' the same form as distribution functions. The negative log-likelihood is a
#' wrapper for \code{\link[evmix:lnormgpdcon]{lnormgpdcon}}, designed towards making
...
#' 
#' Unlike the distribution functions \code{\link[evmix:normgpdcon]{normgpdcon}} the 
...
#' 
#' The function \code{\link[evmix:lnormgpdcon]{lnormgpdcon}} carries out the calculations
#' for the log-likelihood directly, which can be exponentiated to give actual
#' likelihood using (\code{log=FALSE}).
#' 
#' @return \code{\link[evmix:lnormgpdcon]{lnormgpdcon}} gives (log-)likelihood and 
#' \code{\link[evmix:lnormgpdcon]{nlnormgpdcon}} gives the negative log-likelihood.
#'  
...
#' \code{\link[evmix:lnormgpdcon]{lnormgpdcon}} and \code{\link[evmix:lnormgpdcon]{nlnormgpdcon}}. 
#' The \code{\link[evmix:lnormgpdcon]{lnormgpdcon}} also has the usual defaults for
#' the other parameters, but \code{\link[evmix:lnormgpdcon]{nlnormgpdcon}} has no defaults.
#' 
...
#'
#' @seealso \code{\link[evmix:lnormgpd]{lnormgpd}}, \code{\link[evmix:lgpd]{lgpd}} and \code{\link[evmix:gpd]{gpd}}
#' @aliases  lnormgpdcon nlnormgpdcon
#' @family   normgpdcon

# log-likelihood function for normal bulk with GPD for upper tail with a continuity constraint
...
lnormgpdcon <- function(x, nmean = 0, nsd = 1, u = qnorm(0.9, nmean, nsd),
  ...
  lparams = c(length(nmean), length(nsd), length(u), length(xi), length(phiu))
