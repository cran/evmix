#' @export
#' 
#' @title Log-likelihood of Weibull Bulk and GPD Tail Extreme Value Mixture Model
#'
...
#' 
#' They are designed to be used for MLE in \code{\link[evmix:fweibullgpd]{fweibullgpd}}
#' but are available for wider usage, e.g. constructing your own extreme value
#' mixture models.
#' 
#' See \code{\link[evmix:fweibullgpd]{fweibullgpd}} and \code{\link[evmix:fgpd]{fgpd}}
#' for full details.
#' 
#' Log-likelihood calculations are carried out in
#' \code{\link[evmix:lweibullgpd]{lweibullgpd}}, which takes parameters as inputs in
#' the same form as distribution functions. The negative log-likelihood is a
#' wrapper for \code{\link[evmix:lweibullgpd]{lweibullgpd}}, designed towards making
...
#' 
#' Unlike the distribution functions \code{\link[evmix:weibullgpd]{weibullgpd}} the 
...
#' 
#' The function \code{\link[evmix:lweibullgpd]{lweibullgpd}} carries out the calculations
#' for the log-likelihood directly, which can be exponentiated to give actual
#' likelihood using (\code{log=FALSE}).
#' 
#' @return \code{\link[evmix:lweibullgpd]{lweibullgpd}} gives (log-)likelihood and 
#' \code{\link[evmix:lweibullgpd]{nlweibullgpd}} gives the negative log-likelihood.
#'  
...
#' \code{\link[evmix:lweibullgpd]{lweibullgpd}} and \code{\link[evmix:lweibullgpd]{nlnormgpd}}. 
#' The \code{\link[evmix:lweibullgpd]{lweibullgpd}} also has the usual defaults for
#' the other parameters, but \code{\link[evmix:lweibullgpd]{nlweibullgpd}} has no defaults.
#' 
...
lweibullgpd <- function(x, wshape = 1, wscale = 1, u = qweibull(0.9, wshape, wscale),
  ...
  lparams = c(length(wshape), length(wscale), length(u), length(sigmau), length(xi), length(phiu))
  ...
  if ((wscale <= 0) | (wshape <= 0) | (u <= 0) | (sigmau <= 0) | (u <= min(x)) | (u >= max(x))) {
    l = -Inf
  } else {
    if (is.logical(phiu)) {
      if (phiu) {
