...
# inverse cumulative distribution function for hybrid Pareto model with a single continuity constraint
qhpdcon <- function(p, nmean = 0, nsd = 1, u = qnorm(0.9, nmean, nsd), xi = 0, lower.tail = TRUE) {
  ...
  linputs = c(length(p), length(nmean), length(nsd), length(u), length(xi))
  n = max(linputs)
